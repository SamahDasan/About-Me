...
let typeFood=prompt('Did you know what my favourite food?','please enter yes or no').toLowerCase();
function food(type)
{
  while (typeFood !== 'yes' && typeFood !== 'y' && typeFood !== 'no' && typeFood !== 'n') {
    typeFood = prompt('Did you know what my favourite food ? please answer yes or no  ', 'yes/y or no/n').toLowerCase();
  }

  if (typeFood === 'yes' || typeFood === 'y') {
    alert('Perfect!');

  } else if (typeFood === 'no' || typeFood === 'n') {
    alert('Okay! My favourite food is pizza');
  }
}
food(typeFood);
...
if (num==25) alert('Yes the secret number is 25 ^_^');*/
  ...
  function yesno()
  {
    let question1=prompt('Answer About the question by Yes,No ,Y,N \n iS my name is Samah ?');
    ...
    let question2=prompt('Do I like healty food ?');
    else {
    ...
    if(question2.toLowerCase()==='YES'.toLowerCase() || question2.toLowerCase()==='Y'.toLowerCase() || question2.toLowerCase()==='NO'.toLowerCase() || question2.toLowerCase()==='N'.toLowerCase())
      let question3=prompt('Am I married');
    else {
      alert('You Must Enter : yes,no ,y or  n no case sensitive Start Agin');
      yesno();}
...
function game2(){
  ...
  let questions=['What place I preferred to visit Italy or Britin or Japan ??',
  ...
  let answeres1=['italy','britin','japan'];
  let answeres2=['healty','spicy'];
  let answeres3=['painting','animating'];
  let answeres4=['comedy','horror'];
  let answeres5=['Science','History'];
